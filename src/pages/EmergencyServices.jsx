...
function EmergencyServices() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (formData.id) {
        ...
        await updateDoc(doc(db, "EmergencyServices", id), updateData);
        console.log("Item updated:", id);
      } else {
        const newRef = doc(collection(db, "EmergencyServices"));
  ...
  const handleDelete = async () => {
    try {
      await deleteDoc(doc(db, "EmergencyServices", formData.id));
        ...
        <div className="container py-1">
          <div className="row justify-content-center py-1">
            <div className="col-lg-10 pt-lg-1 mt-lg-5 text-center">
              <h1 className="display-3 text-white animated slideInDown">Emergency Services</h1>
              <nav aria-label="breadcrumb">
                <ol className="breadcrumb justify-content-center">
                  <li className="breadcrumb-item"><a href="/">Home</a></li>
                  <li className="breadcrumb-item text-white active" aria-current="page">Emergency Services</li>
                </ol>
              </nav>
            </div>
          </div>
